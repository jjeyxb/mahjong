...
輸入是 :func:`~majsoul_copilot.vision.tiles.hand.read_hand` 給的牌框影像,
輸出是雀魂記法的牌名(``1m``~``9m``、``0m`` 赤五、``1z``~``7z``)。
要轉成 MJAI 用 :func:`~majsoul_copilot.mjai.tiles.ms_to_mjai`。
...
* **模板要對得上牌面皮膚。** 雀魂的 ``mjpface_*`` 是可換的牌面皮膚,換了就要
  ...
  顏色了(見 :mod:`~majsoul_copilot.vision.tiles.hand`)。
...
from majsoul_copilot.vision.tiles.hand import Hand
...
def classify(tile: np.ndarray, templates: TemplateSet) -> Match:
    ...
    Args:
        tile: 一張牌的 BGR 影像,就是 :class:`~majsoul_copilot.vision.tiles.hand.Hand`
...
def classify_hand(
    ...
    Args:
        ...
        hand: :func:`~majsoul_copilot.vision.tiles.hand.read_hand` 的結果。
        templates: 模板集。
