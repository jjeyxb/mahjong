...
from majsoul_copilot.capture.base import BackendUnavailableError, CaptureBackend
from majsoul_copilot.capture.mss_fallback import MSSCaptureBackend
from majsoul_copilot.config.models import BackendName, CaptureConfig
...
def _make(name: BackendName, config: CaptureConfig) -> CaptureBackend:
    if name == "macos":
        from majsoul_copilot.capture.macos import MacOSCaptureBackend

        return MacOSCaptureBackend(retina=config.retina)
    if name == "windows":
        from majsoul_copilot.capture.windows import WindowsCaptureBackend
