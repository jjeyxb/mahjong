...
> ⚠️ **已知問題**：校正在不同畫面（和了動畫、角色立繪）之間會跳，
> 216 幀的錄影會產生 21 種不同的 `table_rect`。修法是「只校正一次然後鎖住」，
> 列為 M3 的第一項。詳見 [docs/decisions.md](docs/decisions.md) 第二節。
...
1. **修校正不穩** —— 目前 216 幀會產生 21 種 `table_rect`。改成校正一次後鎖住，
   加眾數過濾。這是所有 CV 的前提，排第一。
...
| 4 | **校正在幀與幀之間會跳** | 校正只做一次然後鎖住 + 眾數過濾 | ❌ **未修，M3 第一項** |
