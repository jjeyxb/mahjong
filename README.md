...
App 由**三個互相獨立的功能**組成，各自有一個開關：

| # | 功能 | 狀態來源 | 現況 |
|---|---|---|---|
| 1 | **手牌辨識 + 向聽計算** | 螢幕擷取 + CV（只需自己的手牌） | ✅ 完成，已實機驗證 |
| 2 | **麻將 AI 建議** | WebSocket 封包解析 → MJAI → Mortal | ✅ 完成，已實機打過四場 |
| 3 | **放銃分析** | WebSocket 封包 → 整桌公開資訊 → 排除法 | ✅ 完成，已實機驗證 |

三者刻意解耦：功能 1 只吃畫面，功能 2 與 3 吃同一條封包但**只有 2 會載
130MB 權重**——所以只想看放銃危險度的人不必付那個代價。任何一個做不完都不會
擋住其他兩個。

離線的部分是 **M8：打法風格微調**（Mortal fine-tuning，產出一份 `.pth`），
可行性已調查、尚未動工，被「沒有公開的 GRP 權重」擋著。

> ⚠ **「功能 3」在舊文件裡有兩個意思。** App 的第三個功能是**放銃分析**；
> M8 的「功能 3」是**打法風格微調**。兩者完全不同，講的時候請用名字不要用編號。
...
**功能 3（放銃分析）是第二個差異點。** 市面上的輔助工具給的是「這張牌放銃率
12.3%」這類數字，而那要有夠大的牌譜語料統計才有意義。本專題反過來，只輸出
**能從規則推論出來的東西**：一張牌還有幾種待牌型沒被振聽、筋、壁排除掉。
每一條都能指回它憑什麼，而「安全」這兩個字有唯一的意思 —— 規則上不可能。
設計理由與被真實牌譜推翻過兩次的經過，見
[docs/decisions.md](docs/decisions.md) 第十八節。
...
功能 2   WebSocket frame ──liqi──▶ MjaiEvent ──▶ engine × N ──▶ Advice
         (base64 .jsonl)           (JSON)                ↑ 載 130MB 權重

功能 3   WebSocket frame ──liqi──▶ MjaiEvent ──▶ TableTracker ──assess──▶ DangerReport
         (同一條，不重複解析)                    整桌公開資訊    純排除法，不載引擎
```

功能 2 與 3 共用同一條封包串流，但**分成兩條執行緒、兩個開關** —— 因為代價
差了三個數量級：一個要載權重開子程序，一個只是把事件流餵進一個 dataclass。

即時模式下各跑一條執行緒，交會點只有一個郵箱：

```
擷取執行緒(15 fps)  ──┐
封包→引擎(事件驅動)  ├─▶ UpdateBus ──▶ pump()(UI 執行緒)──▶ ViewModel ──▶ Panel
封包→危險度(事件驅動)┘                                                  └─▶ Overlay
```
...
| M7 | Overlay 模式 | ✅ 完成 —— 疊在遊戲上的精簡 HUD,可拖曳 / 鎖定穿透。2026-08-17 已實機疊在對局上驗過 |
| M7-1 | **放銃分析** | ✅ 完成 —— 排除法算每張牌的危險度,獨立頁面 + 可選的 Overlay 清單。見下方「放銃分析實測」 |
| M8 | **打法風格微調**(見下節) | ❌ 未動工,被沒有公開的 GRP 權重擋著 |

目前 **1121 個測試**、ruff + mypy 乾淨。
...
兩個引擎彼此只有一半左右一致 —— 這個差距正是 M8 要調的東西。
...
### 放銃分析實測（2026-08-14 量、2026-08-17 實機驗證）

全部量自 `tests/fixtures/real_game_full.jsonl` —— 一場完整東風戰，259 次捨牌。

**這個模組不輸出機率。** 「放銃率 12.3%」要有夠大的語料統計才有意義，手上
只有兩場，差三四個數量級。硬湊公式生出百分比會得到一個**看起來精確、實際上
沒有來源**的數字。所以輸出的是**還有幾種待牌型沒被排除**：

| 排除的依據 | 是規則還是估計 |
|---|---|
| 對方打過這張 → 振聽 | 規則 |
| 那一型也會和的另一張他打過 → **筋** | 規則（筋是自然掉出來的，沒有另外寫一條） |
| 那一型需要的牌四張全見 → **壁** | 規則 |
| 立直之後別人打過的 | 規則（**唯一一條是立直專屬的**） |

`安全` 因此有唯一的意思：**對三家都不可能放銃**，而不是「機率很低」。

#### 被真實牌譜推翻兩次

| # | 第一版怎麼寫 | 資料說了什麼 |
|---|---|---|
| 1 | 只評估**立直家** | 三次榮和裡**兩次**是沒立直的人和的。對其中一次給出「8s 安全 —— 現物」，而那正是放銃牌 |
| 2 | 只認立直算威脅 | 三副露的曝光量與立直是同一量級（**53 對 59** 個捨牌時點），而最重的一次榮和正是一個三副露、沒立直的人 |

修法分別是：評估**三家對手**（上表四條只有最後一條是立直專屬的）；
把三副露也算成威脅，**但只影響「指名誰」，不影響危險度等級** ——
等級由排除法數出來、副露數是估計，混在一起畫面會出現「非常危險」配著
「還有 2 型」這種自相矛盾的東西。
...
| 　　同上：門檻 2 副露 | 132/259（51%）—— 標籤開始貶值，未採用 |
| 每張牌的危險度分布 | 安全 4.2% / 比較安全 40.9% / 危險 35.1% / 非常危險 19.7% |

**已知覆蓋缺口**：我們自己那次放銃的和牌者只有 2 副露，現行門檻 3 抓不到他。
「2 副露 + 役牌」比單純的副露數硬，留作獨立一項。另外這個模組**不看打點** ——
放銃給門清立直 dora 3 與給 1000 點的仕掛け是同一級。

完整設計理由見 [docs/decisions.md](docs/decisions.md) 第十八節。
